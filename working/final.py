...
def write_to_oled(text):
    """
    Writes the current time and day of the week as the first line on the OLED display,
    followed by the given text. Wraps text if it exceeds the screen width.
    """
    # Fetch current time and day of the week from a free API
    try:
        ...
        if response.status_code == 200:
            ...
            current_time = dt.strftime("%H:%M")  # Hours and minutes
    ...
    # Add time and day as the first line
    time_line = f"{day_of_week} {current_time}"
    draw.text((0, 0), time_line, font=font, fill=255)
    ...
    # Draw the remaining lines below the time and day
    y = 15  # Start below the time and day line
    for line in lines:
...
def record_and_display():
    ...
    try:
        ...
        response = {"status": "success", "transcription": text_only}
    except Exception as e:
        print("Error:", e)
